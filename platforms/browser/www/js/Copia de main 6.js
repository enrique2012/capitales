var app = {
	...
	finJuego: function(){
		...
		html+='<table>';
		html+='<tr><th id = "nombre"></th><td id = "hiscore"></td></tr>';
		html+='</table>';
		...
		nombre.innerHTML = snapshot.val();
		
		
		 
	 	});
		
		//lee puntos en firebase
		ref.child('puntos').once('value', function(snapshot){
		var hiscore = document.querySelector('#hiscore');
		hiscore.innerHTML = snapshot.val();
		 
		});
		
		var hiscore = document.querySelector('#hiscore').innerHTML;
		console.log(hiscore);
		if (totalPuntos > hiscore){
			alert('nuevo record');
		}else{
