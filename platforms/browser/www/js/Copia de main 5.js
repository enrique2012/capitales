var app = {
	...
	grabarRecords: function(){
		localStorage.setItem("records",JSON.stringify(app.model));
	},
	...
	finJuego: function(){
		...
		var guardado = JSON.parse(localStorage.getItem("records"));
		
		
		html+='<hr>';
		html+='<h3 class="centraTexto">HISCORE:</h3>';
		html+='<table>';
		html+='<tr><th>'+ guardado.hiScores[0].nombre +'</th><td>'+ guardado.hiScores[0].hiScore + '</td></tr>';
		html+='<tr><th>'+ guardado.hiScores[1].nombre +'</th><td>'+ guardado.hiScores[1].hiScore + '</td></tr>';
		html+='<tr><th>'+ guardado.hiScores[2].nombre +'</th><td>'+ guardado.hiScores[2].hiScore + '</td></tr>';
		html+='</table>';
